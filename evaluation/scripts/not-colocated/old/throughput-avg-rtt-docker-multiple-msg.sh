...
output_file="metrics/throughput_rtt_docker_batch_log.csv"
...
for ((i=1; i<=runs; i++)); do
    ...
    # Run Docker Compose for client/server setup
    output=$(docker compose up --build server client --abort-on-container-exit 2>&1)
    docker compose down --volumes --remove-orphans

    # --- Extract Throughput ---
    throughput_line=$(echo "$output" | grep "\[METRICS\]" | grep "THROUGHPUT =")
    ...
    rtt_lines=$(echo "$output" | grep "\[METRICS\]" | grep "RTT =")
    total_rtt=0
    rtt_count=0

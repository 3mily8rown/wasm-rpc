...
RpcClient::RpcClient() : running_(true) {
    ...
    // persistent_ = std::make_unique<PersistentSocketClient>(get_server_ip(), message_port);
    // persistent_->connect();
...
bool RpcClient::receive(uint32_t expected_tag, T* out_msg) {
    uint8_t* buf = static_cast<uint8_t*>(std::malloc(4096));
    if (!buf) return false;

    int32_t len = receive_rpcresponse(buf, 4096);
...
bool RpcClient::pollResponse(uint32_t expected_tag, uint32_t request_id, T* out_msg) {
    ...
    int32_t len = receive_rpcresponse_with_id(buf, 4096, request_id);
    if (len == 0) {
